...
Vpipeline__Syms::Vpipeline__Syms(VerilatedContext* contextp, const char* namep, Vpipeline* modelp)
    ...
    , TOP__pipeline__my_FE_stage{this, Verilated::catName(namep, "pipeline.my_FE_stage")}
    , TOP__pipeline__my_WB_stage{this, Verilated::catName(namep, "pipeline.my_WB_stage")}
{
    ...
    TOP__pipeline.my_FE_stage = &TOP__pipeline__my_FE_stage;
    TOP__pipeline.my_WB_stage = &TOP__pipeline__my_WB_stage;
    // Setup each module's pointer back to symbol table (for public functions)
    TOP.__Vconfigure(true);
    TOP__pipeline.__Vconfigure(true);
    TOP__pipeline__my_FE_stage.__Vconfigure(true);
    TOP__pipeline__my_WB_stage.__Vconfigure(true);
    // Setup scopes
    __Vscope_pipeline__my_FE_stage.configure(this, name(), "pipeline.my_FE_stage", "my_FE_stage", 0, VerilatedScope::SCOPE_OTHER);
    ...
    for (int __Vfinal = 0; __Vfinal < 2; ++__Vfinal) {
        __Vscope_pipeline__my_FE_stage.varInsert(__Vfinal,"correct_predictions", &(TOP__pipeline__my_FE_stage.correct_predictions), false, VLVT_UINT32,VLVD_NODIR|VLVF_PUB_RW,1 ,31,0);
        __Vscope_pipeline__my_FE_stage.varInsert(__Vfinal,"total_branches", &(TOP__pipeline__my_FE_stage.total_branches), false, VLVT_UINT32,VLVD_NODIR|VLVF_PUB_RW,1 ,31,0);
