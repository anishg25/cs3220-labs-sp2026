...
always @(*) begin 
  ...
  assign is_br_DE  = ((op_I_DE == `BEQ_I) || (op_I_DE == `BNE_I) || (op_I_DE == `BLT_I) || (op_I_DE == `BGE_I) || (op_I_DE == `BLTU_I) || (op_I_DE == `BGEU_I) || (op_I_DE == `JALR_I) || (op_I_DE == `JAL_I)) ? 1 : 0;
  assign wr_reg_DE = ((op_I_DE == `ADD_I) || 
                      (op_I_DE == `ADDI_I) || 
                      (op_I_DE == `ANDI_I) || (op_I_DE == `AUIPC_I) || 
                      (op_I_DE == `LUI_I) || (op_I_DE == `JAL_I) || 
                      (op_I_DE == `JALR_I) || (op_I_DE == `SUB_I) || 
                      (op_I_DE == `SRA_I) || (op_I_DE == `OR_I) ||
                      (op_I_DE == `ORI_I) || (op_I_DE == `SLL_I) || (op_I_DE == `SLLI_I)
